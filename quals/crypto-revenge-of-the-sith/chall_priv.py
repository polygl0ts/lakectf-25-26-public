...
# this is just to check that its solvable.

def decrypt(s, u_batch, v_batch):
    ...
    for b in range(batch_size):
        u = u_batch[b]
        v = v_batch[b]
        m_n = (v - _vec_poly_mul(s, u)) % q
        m_n_centered = np.array([x - q if x > q//2 else x for x in m_n], dtype=int)
        m_b_dec = (np.round(m_n_centered.astype(float) / ((q + 1) // 2)) % 2).astype(int)
        m_b_dec_list.append(m_b_dec)
        
    return np.array(m_b_dec_list)

decrypted_batch = decrypt(s, u, v)

solution_bits = []
for m_poly in decrypted_batch:
    solution_bits.extend(m_poly.tolist())

solution = "".join([
    chr(int("".join(map(str, solution_bits[i:i+8])), 2)) 
    for i in range(0, len(solution_bits), 8)
])

assert flag == solution